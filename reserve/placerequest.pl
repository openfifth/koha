...
if ( $op eq 'cud-placerequest' && $patron ) {
    ...
    # By default a hold group counts as one unit. Run the count check once (first
    # hold) and skip it for subsequent holds, unless count_individually is active.
    my $count_policy = $hold_group_param
        ...
        : undef;
    my $skip_count_for_group = $hold_group_param
        && !( $count_policy && $count_policy->rule_value eq 'count_individually' );
        ...
        if (@checkitems) {
            ...
            for ( my $i = 0 ; $i < scalar @checkitems ; $i++ ) {
                ...
                if ( my $item_pickup_location = $input->param("item_pickup_$checkitem") ) {
                    ...
                    my $check_params = {};
                    $check_params->{hold_group_count_checked} = 1
            ...
            my $book_params = {};
            $book_params->{hold_group_count_checked} = 1
                if $skip_count_for_group && $group_count_checked;
            if ( $can_override
