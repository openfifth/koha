                ...
                foreach my $barcode (@barcodes) {
                    ...
                    if ($item) {
                        ...
                        if ($@) {
                            ...
                            push @messages,
                                { item_barcode => $barcode, type => 'message', code => 'success_on_add_biblio' };
                        } else {
                            push @messages,
                                { item_barcode => $barcode, type => 'message', code => 'error_on_add_biblio' };
                        }
                    } else {
                ...
                foreach my $biblionumber (@biblionumbers) {
                    ...
                    if ( defined $biblio ) {
                        ...
                        if ($@) {
                            ...
                            push @messages,
                                { bibnum => $biblionumber, type => 'message', code => 'success_on_add_biblio' };
                        } else {
                            push @messages,
                                { bibnum => $biblionumber, type => 'message', code => 'error_on_add_biblio' };
                        }
                    } else {
    ...
    if ($shelf) {
        if ( $shelf->can_be_viewed($loggedinuser) ) {
            ...
            while ( my $content = $contents->next ) {
                ...
                $this_item->{imageurl} =
                    $itemtype ? C4::Koha::getitemtypeimagelocation( 'intranet', $itemtype->imageurl ) : q{};
                $this_item->{description} =
                    $itemtype ? $itemtype->description : q{};    #FIXME Should this be translated_description ?
