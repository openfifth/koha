...
is(
    $retrieved_patron_1->cardnumber, $new_patron_1->cardnumber,
    'Find a patron by borrowernumber should return the correct patron'
);
...
subtest 'library' => sub {
    ...
    is(
        ref( $retrieved_patron_1->library ), 'Koha::Library',
        'Koha::Patron->library should return a Koha::Library object'
    );
};
...
subtest 'sms_provider' => sub {
    ...
    is_deeply(
        $retrieved_patron_1->sms_provider->unblessed, $sms_provider,
        'Koha::Patron->sms_provider returns the correct SMS provider'
    );
    is(
        ref( $retrieved_patron_1->sms_provider ), 'Koha::SMS::Provider',
        'Koha::Patron->sms_provider should return a Koha::SMS::Provider object'
    );
};
...
subtest 'guarantees' => sub {
    ...
    is(
        ref($guarantees), 'Koha::Patron::Relationships',
        'Koha::Patron->guarantees should return a Koha::Patrons result set in a scalar context'
    );
    ...
    my $relationship_1 = Koha::Patron::Relationship->new(
        { guarantor_id => $new_patron_1->id, guarantee_id => $guarantee_1->{borrowernumber}, relationship => 'test' } )
        ->store();
    my $guarantee_2    = $builder->build( { source => 'Borrower' } );
    my $relationship_2 = Koha::Patron::Relationship->new(
        { guarantor_id => $new_patron_1->id, guarantee_id => $guarantee_2->{borrowernumber}, relationship => 'test' } )
        ->store();

    $guarantees = $new_patron_1->guarantee_relationships;
    is(
        ref($guarantees), 'Koha::Patron::Relationships',
        'Koha::Patron->guarantee_relationships should return a Koha::Patrons result set in a scalar context'
    );
...
subtest 'siblings' => sub {
    ...
    my $relationship_1 = Koha::Patron::Relationship->new(
        {
            guarantor_id => $new_patron_1->borrowernumber, guarantee_id => $guarantee_1->{borrowernumber},
            relationship => 'test'
        }
    )->store();
    ...
    is(
        ...
        'Koha::Patron->siblings should return a Koha::Patrons result set in a scalar context'
    );
    is( $siblings->count, 0, 'guarantee_1 should not have siblings yet' );
    my $guarantee_2    = $builder->build( { source => 'Borrower' } );
    my $relationship_2 = Koha::Patron::Relationship->new(
        {
            guarantor_id => $new_patron_1->borrowernumber, guarantee_id => $guarantee_2->{borrowernumber},
            relationship => 'test'
        }
    )->store();
    ...
    my $relationship_3 = Koha::Patron::Relationship->new(
        {
            guarantor_id => $new_patron_1->borrowernumber, guarantee_id => $guarantee_3->{borrowernumber},
            relationship => 'test'
        }
    )->store();
    $siblings = $retrieved_guarantee_1->siblings;
...
subtest 'has_overdues' => sub {
    ...
    my $issue    = Koha::Checkout->new(
        {
            borrowernumber => $new_patron_1->id, itemnumber => $item_1->itemnumber, date_due => $tomorrow,
            branchcode     => $library->{branchcode}
        }
    )->store();
    is( $retrieved_patron->has_overdues, 0, );
    ...
    $issue = Koha::Checkout->new(
        {
            borrowernumber => $new_patron_1->id, itemnumber => $item_1->itemnumber, date_due => $yesterday,
            branchcode     => $library->{branchcode}
        }
    )->store();
...
subtest 'is_going_to_expire' => sub {
    ...
    is(
        ...
        'Patron should not be considered going to expire if dateexpiry is today and pref is 0'
    );
    ...
    is(
        $patron->is_going_to_expire, 0,
        'Patron should not be considered going to expire if dateexpiry is 11 days ahead and pref is 10'
    );
    ...
    is(
        $patron->is_going_to_expire, 0,
        'Patron should not be considered going to expire if dateexpiry is 10 days ahead and pref is 0'
    );
    ...
    is(
        $patron->is_going_to_expire, 0,
        'Patron should not be considered going to expire if dateexpiry is 20 days ahead and pref is 10'
    );
    ...
    is(
        ...
        'Patron should be considered going to expire if dateexpiry is 10 days ahead and pref is 20'
    );
...
subtest 'renew_account' => sub {
    ...
    for my $date ( '2016-03-31', '2016-11-30', '2019-01-31', dt_from_string() ) {
        ...
        my $a_year_later_minus_a_month =
            $a_month_ago->clone->add( months => 12, end_of_month => 'limit' )->truncate( to => 'day' );
        my $a_month_later = $dt->clone->add( months => 1, end_of_month => 'limit' )->truncate( to => 'day' );
        my $a_year_later_plus_a_month =
            $a_month_later->clone->add( months => 12, end_of_month => 'limit' )->truncate( to => 'day' );
        ...
        is(
            $retrieved_patron->date_renewed, undef,
            "Date renewed is not set for patrons that have never been renewed"
        );
        ...
        is(
            $retrieved_patron->date_renewed, output_pref( { dt => $dt, dateformat => 'iso', dateonly => 1 } ),
            "Date renewed is set when calling renew_account"
        );
        ...
        is(
            dt_from_string($retrieved_expiry_date), $a_year_later_plus_a_month,
            "$a_month_later + 12 months must be $a_year_later_plus_a_month"
        );
...
subtest "move_to_deleted" => sub {
    ...
    my $retrieved_patron = Koha::Patrons->find( $patron->{borrowernumber} );
    is(
        ref( $retrieved_patron->move_to_deleted ), 'Koha::Schema::Result::Deletedborrower',
        'Koha::Patron->move_to_deleted should return the Deleted patron'
    );    # FIXME This should be Koha::Deleted::Patron
    my $deleted_patron = $schema->resultset('Deletedborrower')->search(
    ...
    isnt(
        $deleted_patron->{updated_on}, $retrieved_patron->updated_on,
        'Koha::Patron->move_to_deleted should have correctly updated the updated_on column'
    );
    $deleted_patron->{updated_on} = $originally_updated_on;    #reset for simplicity in comparing all other fields
    is_deeply(
        $deleted_patron, $patron,
        'Koha::Patron->move_to_deleted should have correctly moved the patron to the deleted table'
    );
...
subtest "delete" => sub {
    ...
    my $modification = $builder->build_object(
        { class => 'Koha::Patron::Modifications', value => { borrowernumber => $patron->borrowernumber } } );
    ...
    $list_to_share->share("valid key")->accept( "valid key", $staff_patron->borrowernumber )
        ;    # this share should be removed at deletion too
    my $deleted = $patron->delete;
    is(
        ref($deleted), 'Koha::Patron',
        'Koha::Patron->delete should return the deleted patron object if the patron has been correctly deleted'
    );
    ...
    is(
        Koha::Old::Holds->search( { reserve_id => $hold->reserve_id } )->count, 1,
        q|Koha::Patron->delete should have cancelled patron's holds|
    );

    is(
        Koha::Holds->search( { borrowernumber => $patron->borrowernumber } )->count, 0,
    ...
    is(
        Koha::Virtualshelfshares->search( { borrowernumber => $staff_patron->borrowernumber } )->count, 0,
        "New owner of list should have shares removed"
    );
    is(
        Koha::Virtualshelfshares->search( { borrowernumber => $patron_for_sharing->borrowernumber } )->count, 1,
    ...
    is(
        Koha::Patron::Modifications->search( { borrowernumber => $patron->borrowernumber } )->count, 0,
...
subtest 'Koha::Patrons->delete' => sub {
    ...
    my $set     = Koha::Patrons->search(
        { borrowernumber => { -in => [ $patron1->borrowernumber, $patron2->borrowernumber ] } } );
    is( $set->count,                   2, 'Two patrons found as expected' );
    is( $set->delete( { move => 1 } ), 2, 'Two patrons deleted' );
    my $deleted_patrons = Koha::Old::Patrons->search(
        { borrowernumber => { -in => [ $patron1->borrowernumber, $patron2->borrowernumber ] } } );
...
subtest 'add_enrolment_fee_if_needed' => sub {
    ...
    $total = $patron->account->balance;
    is(
        int($total), int( $enrolmentfee_K + $enrolmentfee_J ),
        "Kid growing and become a juvenile, they should pay " . ( $enrolmentfee_K + $enrolmentfee_J )
    );
    ...
    is(
        ...
        "Juvenile growing and become an young adult, they should pay "
...
subtest 'checkouts + pending_checkouts + overdues + old_checkouts' => sub {
    ...
    is(
        $first_checkout->unblessed_all_relateds->{biblionumber}, $item_2->biblionumber,
...
subtest 'get_routing_lists' => sub {
    ...
    is(
        ref($routinglists), 'Koha::Subscription::Routinglists',
        'get_routing_lists returns Koha::Subscription::Routinglists'
    );
...
subtest 'is_valid_age' => sub {
    ...
    my @dates = (
        {
            today         => '2020-02-28',
            add_m12_m6_m1 => { date => '2007-08-27', expected_age => 12, valid => 0 },
            add_m3_m6_m1  => { date => '2016-08-27', expected_age => 3,  valid => 0 },
            add_m7_m6_m1  => { date => '2013-02-28', expected_age => 7,  valid => 1 },
            add_m5_0_0    => { date => '2015-02-28', expected_age => 5,  valid => 1 },
            add_m5_0_p1   => { date => '2015-03-01', expected_age => 4,  valid => 0 },
            add_m5_0_m1   => { date => '2015-02-27', expected_age => 5,  valid => 1 },
            add_m11_0_0   => { date => '2009-02-28', expected_age => 11, valid => 0 },
            add_m11_0_p1  => { date => '2009-03-01', expected_age => 10, valid => 1 },
            add_m11_0_m1  => { date => '2009-02-27', expected_age => 11, valid => 0 },
        },
    );
...
subtest 'search_upcoming_membership_expires' => sub {
    ...
    is(
        $upcoming_mem_expires->count, $nb_of_expires + 1,
        'Get upcoming membership expires should return one new borrower.'
    );

    # Test with branch
    $upcoming_mem_expires =
        Koha::Patrons->search_upcoming_membership_expires( { 'me.branchcode' => $library->{branchcode} } );
    ...
    $upcoming_mem_expires =
        Koha::Patrons->search_upcoming_membership_expires( { 'me.branchcode' => $library->{branchcode} } );
    is(
        $upcoming_mem_expires->count, 0,
        'Get upcoming membership expires with MembershipExpiryDaysNotice==0 should not return new records.'
    );
    ...
    is(
        $upcoming_mem_expires->count, 0,
        'Get upcoming membership expires without MembershipExpiryDaysNotice should not return new records.'
    );

    # Test the before parameter
    t::lib::Mocks::mock_preference( 'MembershipExpiryDaysNotice', 15 );
    $upcoming_mem_expires = Koha::Patrons->search_upcoming_membership_expires(
        { 'me.branchcode' => $library->{branchcode}, before => $nb_of_days_before } );
    is( $upcoming_mem_expires->count, 2, 'Expect two results for before' );

    # Test after parameter also
    $upcoming_mem_expires = Koha::Patrons->search_upcoming_membership_expires(
        { 'me.branchcode' => $library->{branchcode}, before => $nb_of_days_before, after => $nb_of_days_after } );
    is( $upcoming_mem_expires->count, 3, 'Expect three results when adding after' );
    Koha::Patrons->search(
        {
            borrowernumber =>
                { in => [ $patron_1->{borrowernumber}, $patron_2->{borrowernumber}, $patron_3->{borrowernumber} ] }
        }
    )->delete;
};
...
subtest 'search_patrons_to_anonymise' => sub {
    ...
    subtest 'patron privacy is 1 (default)' => sub {
        ...
        my ( $returned_1, undef, undef ) =
            C4::Circulation::AddReturn( $item_1->barcode, undef, undef, dt_from_string('2010-10-10') );
        my ( $returned_2, undef, undef ) =
            C4::Circulation::AddReturn( $item_2->barcode, undef, undef, dt_from_string('2011-11-11') );
        is( $returned_1 && $returned_2, 1, 'The items should have been returned' );

        my $patrons_to_anonymise = Koha::Patrons->search_patrons_to_anonymise( { before => '2010-10-11' } )
        ...
        my $rows_affected = Koha::Old::Checkouts->search(
            {
                borrowernumber => [
                    Koha::Patrons->search_patrons_to_anonymise( { before => '2011-11-12' } )
        ...
        is(
            $patrons_to_anonymise->count, 0,
            'search_patrons_to_anonymise should return 0 after anonymisation is done'
        );
        ...
        $rows_affected = Koha::Old::Checkouts->search(
            { borrowernumber => [ Koha::Patrons->search_patrons_to_anonymise->get_column('borrowernumber') ] } )
            ->anonymize;
        $sth->execute( $item_1->itemnumber );
        ($borrowernumber_used_to_anonymised) = $sth->fetchrow_array;
        is(
    ...
    subtest 'patron privacy is 0 (forever)' => sub {
        ...
        my ( $returned, undef, undef ) =
            C4::Circulation::AddReturn( $item->barcode, undef, undef, dt_from_string('2010-10-10') );
        is( $returned, 1, 'The item should have been returned' );

        my $dbh = C4::Context->dbh;
        my ($borrowernumber_used_to_anonymised) = $dbh->selectrow_array(
            q|
...
subtest 'account_locked' => sub {
    ...
    is(
        $patron->account_locked, 1,
        'Patron could not have 4 failed attempts, but account should still be considered locked'
    );
    $patron->login_attempts(-1)->store;
...
subtest 'Test Koha::Patrons::merge' => sub {
    ...
    is(
        Koha::Patrons->search( { borrowernumber => $keeper->id } )->count, 1,
        "Patron from attempted merge with AnonymousPatron still exists"
    );
    ...
    subtest 'extended attributes' => sub {
        ...
        sub compare_attributes {
            ...
            is_deeply(
                [ sort $got->search( { code => $code } )->get_column('attribute') ],
                $expected
            );
        }
        compare_attributes(
        ...
        $keep_patron  = $builder->build_object( { class => 'Koha::Patrons' } );
...
subtest '->store' => sub {
    ...
    subtest 'skip updated_on for BorrowersLog' => sub {
        ...
        is( $logs->count, 0, '->store should not have generated a log for updated_on' )
            or diag 'Log generated:' . Dumper( $logs->unblessed );
        $schema->storage->txn_rollback;
    };
    ...
    subtest 'create user usage' => sub {
        ...
        my $queued_notices = Koha::Notice::Messages->search( { borrowernumber => $new_patron->borrowernumber } );
        is(
...
subtest '->set_password' => sub {
    ...
    is( $THE_notice->status, 'failed', "The notice was handled immediately and failed on wrong email address." )
        ;    #FIXME Mock sending mail
    $schema->storage->txn_rollback;
};
...
subtest 'search_unsubscribed' => sub {
    ...
    Koha::Patron::Consent->new(
        { borrowernumber => $patron1->borrowernumber, type => 'GDPR_PROCESSING', refused_on => dt_from_string } )
        ->store;
    ...
    Koha::Patron::Consent->new(
        {
            borrowernumber => $patron2->borrowernumber, type => 'GDPR_PROCESSING',
            refused_on     => dt_from_string->subtract( days => 2 )
        }
    )->store;
...
subtest 'anonymize' => sub {
    ...
    my $issue = $builder->build_object(
        { class => 'Koha::Checkouts', value => { borrowernumber => $patron2->borrowernumber } } );
...
subtest 'queue_notice' => sub {
    ...
    is(
        ...
        "Count of queued notices went up by one"
    );
    ...
    is(
        $patron->queue_notice(
            { letter_params => $letter_params, message_transports => \@mtts, message_name => 'Hold_Filled' }
        ),
        undef,
        "Nothing done if transports and name sent"
    );
...
subtest 'filter_by_amount_owed' => sub {
    ...
    $filtered = $results->filter_by_amount_owed( { less_than => $upper_limit } );
    is(
        $filtered->_resultset->as_subselect_rs->count, 2,
        "filter_by_amount_owed({ less_than => $upper_limit }) found two patrons"
    );

    # Catch user with 1 x 8.00 and 1 x 10.00 fine
    $filtered = $results->filter_by_amount_owed( { more_than => $lower_limit } );
    is(
        $filtered->_resultset->as_subselect_rs->count, 1,
    ...
    $filtered = $results->filter_by_amount_owed( { more_than => $lower_limit, less_than => $upper_limit } );
    is(
        $filtered->_resultset->as_subselect_rs->count, 0,
...
subtest 'libraries_where_can_edit_items() and can_edit_items_from() tests' => sub {
    ...
    my $sth =
        C4::Context->dbh->prepare(q|INSERT INTO user_permissions( borrowernumber, module_bit, code ) VALUES (?, 9, ?)|)
        ;    # 9 for editcatalogue
             # 2 patrons from library_1A (group1)
             # patron_1A_1 see patron's infos from outside its group
             # Setting flags => undef to not be considered as superlibrarian
    my $patron_1A_1 = $builder->build(
        { source => 'Borrower', value => { branchcode => $library_1A->branchcode, flags => undef, } } );
    ...
    my $patron_1B = $builder->build(
        { source => 'Borrower', value => { branchcode => $library_1B->branchcode, flags => undef, } } );
    ...
    my $patron_2A = $builder->build(
        { source => 'Borrower', value => { branchcode => $library_2A->branchcode, flags => undef, } } );
    ...
    subtest 'libraries_where_can_edit_items' => sub {
        ...
        is_deeply(
            \@branchcodes, [ sort ( $library_1A->branchcode, $library_1B->branchcode ) ],
            "patron_1A_2 doesn't have edit_any_item => Can only edit items from its group"
        );

        @branchcodes = $patron_2A->libraries_where_can_edit_items;
        is_deeply(
            \@branchcodes, [ $library_2A->branchcode ],
            "patron_2A doesn't have edit_any_item => Can only see patron's from its group"
        );
    };
    ...
    subtest 'can_edit_items_from' => sub {
        ...
        is(
            $patron_1A_1->can_edit_items_from( $library_1A->id ), 1,
            "patron_1A_1 can see patron_1A_2, from its library"
        );
        is( $patron_1A_1->can_edit_items_from( $library_1B->id ), 1, "patron_1A_1 can see patron_1B, from its group" );
        is(
            $patron_1A_1->can_edit_items_from( $library_2A->id ), 1,
            "patron_1A_1 can see patron_1A_2, from another group"
        );

        t::lib::Mocks::mock_userenv( { patron => $patron_1A_2 } );
        is(
            $patron_1A_2->can_edit_items_from( $library_1A->id ), 1,
            "patron_1A_2 can see patron_1A_1, from its library"
        );
        is( $patron_1A_2->can_edit_items_from( $library_1B->id ), 1, "patron_1A_2 can see patron_1B, from its group" );
        is(
            $patron_1A_2->can_edit_items_from( $library_2A->id ), 0,
            "patron_1A_2 can NOT see patron_2A, from another group"
        );
    };
};
