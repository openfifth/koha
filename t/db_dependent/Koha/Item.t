...
subtest 'mark_lost' => sub {
    ...
    # mark_lost guards against unrelated accountlines being edited. Confirm this.
    my $other_type  = $item_line->( { debit_type_code => 'LOST' } );
    my $returned    = $item_line->( { status          => 'RETURNED' } );
    my $other_owner = $item_line->( { borrowernumber  => $other_patron->borrowernumber } );

    my $other_item_fine = $builder->build_object(
        {
            class => 'Koha::Account::Lines',
            value => {
                borrowernumber   => $patron->borrowernumber,
                itemnumber       => $builder->build_sample_item->itemnumber,
                debit_type_code  => 'OVERDUE',
                credit_type_code => undef,
                status           => 'UNRETURNED',
            }
    ...
    $other_owner->discard_changes;
    is( $other_owner->status, 'UNRETURNED', "another borrower's line on the same item is left alone" );

    $other_item_fine->discard_changes;
    is( $other_item_fine->status, 'UNRETURNED', "the borrower's line on another item is left alone" );
