...
subtest 'patron() tests' => sub {
    ...
    is(
        $line->borrowernumber, $account_line_patron->borrowernumber,
        'Koha::Account::Line->patron should return the correct borrower'
    );
...
subtest 'manager() tests' => sub {
    ...
    is(
        $line->manager_id, $account_line_manager->borrowernumber,
        'Koha::Account::Line->manager should return the correct staff'
    );
...
subtest 'apply() tests' => sub {
    ...
    throws_ok { $credit->apply( { debits => [ $debits->as_list ] } ); }
    ...
    throws_ok { $debit_1->apply( { debits => [ $debits->as_list ] } ); }
    ...
    throws_ok { $credit_3->apply( { debits => [ $debits->as_list ] } ); }
    ...
    $debits = Koha::Account::Lines->search(
        { accountlines_id => { -in => [ $debit_1->id, $debit_2->id, $debit_3->id, $credit->id ] } } );
    throws_ok {
    ...
    $debits =
        Koha::Account::Lines->search( { accountlines_id => { -in => [ $debit_1->id, $debit_2->id, $debit_3->id ] } } );
    ...
    is(
        $called, 0,
        'C4::Circulation::AddRenew NOT called when RenewAccruingItemWhenPaid enabled but credit type is "FORGIVEN"'
    );
    ...
    my $pay_lost =
        $loser_account->add_credit( { amount => 27.630000, user_id => $loser->id, interface => 'intranet' } );
...
subtest 'checkout() tests' => sub {
    ...
    is(
        $line_checkout->issue_id, $checkout->issue_id,
    ...
    is(
        $old_line_checkout->issue_id, $old_checkout->issue_id,
        'Koha::Account::Line->checkout should return the correct old_checkout'
    );

    $old_line_checkout->delete;
    $line->discard_changes
        ;    #NOTE: discard_changes refreshes the whole resultset, get_from_storage only refreshes the unjoined row
...
subtest "payout() tests" => sub {
    ...
    my @required = ( 'interface', 'staff_id', 'branch', 'payout_type', 'amount' );
...
subtest "reduce() tests" => sub {
    ...
    my $offsets = Koha::Account::Offsets->search( { credit_id => $reduction->id } );
    ...
    $offsets = Koha::Account::Offsets->search( { credit_id => $reduction->id } );
    is( $offsets->count, 2, 'Two offsets generated' );
    $THE_offset = $offsets->next;
...
subtest "cancel() tests" => sub {
    ...
    is(
        $debit2->amountoutstanding + 0,
        15, 'Second fee has amount outstanding of 15'
    );
    throws_ok {
        $credit->cancel( { staff_id => $staff->borrowernumber, branch => $library->branchcode } );
    }
    'Koha::Exceptions::Account::IsNotDebit',
    ...
    throws_ok {
        $debit2->cancel( { staff_id => $staff->borrowernumber, branch => $library->branchcode } );
    }
    'Koha::Exceptions::Account',
        '->cancel() can only be used with debits that have not been offset';

    my $cancellation = $debit1->cancel( { staff_id => $staff->borrowernumber, branch => $library->branchcode } );
    is(
        ref($cancellation), 'Koha::Account::Line',
    ...
    is(
        $cancellation->amountoutstanding() * 1,
    ...
    my $offsets = Koha::Account::Offsets->search( { credit_id => $cancellation->id } );
