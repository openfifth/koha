...
subtest '_read_configuration() tests' => sub {
    ...
    is_deeply(
        $configuration->{nodes}, \@servers,
        'get_elasticsearch_params is just a wrapper for _read_configuration'
    );

    t::lib::Mocks::mock_config(
        'elasticsearch',
        {
            server          => \@servers, index_name => 'index', cxn_pool => 'Sniff', trace_to => 'Stderr',
            request_timeout => 42
        }
    );
...
subtest 'get_elasticsearch_settings() tests' => sub {
    ...
    is(
        $settings->{index}{analysis}{analyzer}{analyzer_phrase}{tokenizer}, 'keyword',
        'Index settings parsed correctly'
    );
};
...
subtest 'get_elasticsearch_mappings() tests' => sub {
    ...
    my $search_engine_elasticsearch = Koha::SearchEngine::Elasticsearch::Search->new(
...
subtest 'Koha::SearchEngine::Elasticsearch::marc_records_to_documents () tests' => sub {
    ...
    is_deeply(
        $docs->[0]->{author}, [ 'Author 1', 'Corp Author' ],
        'First document author field should be set correctly'
    );
    ...
    is_deeply(
        $docs->[0]->{author__sort}, ['Author 1 Corp Author'],
        'First document author__sort field should be set correctly'
    );
    ...
    is_deeply(
        $docs->[0]->{title_wildcard}, [ 'Title:', 'first record' ],
        'First document title_wildcard field should be set correctly'
    );

    is(
        scalar @{ $docs->[0]->{author__suggestion} }, 2,
        'First document author__suggestion field should contain two values'
    );
    is_deeply(
    ...
    is(
        scalar @{ $docs->[0]->{title__suggestion} }, 3,
        'First document title__suggestion field should contain three values'
    );
    is_deeply(
    ...
    is(
        scalar @{ $docs->[0]->{items_withdrawn_status} }, 2,
        'First document items_withdrawn_status field should have two values'
    );
    is_deeply(
    ...
    is(
        scalar @{ $docs->[0]->{type_of_record_and_bib_level} }, 1,
        'First document type_of_record_and_bib_level field should have one value'
    );
    is_deeply(
    ...
    is_deeply(
        $docs->[0]->{isbn}, [ '978-1-56619-909-4', '9781566199094', '1-56619-909-3', '1566199093' ],
        'First document isbn field should be set correctly'
    );
    ...
    # Tests for 'year' type
    is(
        scalar @{ $docs->[0]->{'date-of-publication'} }, 1,
        'First document date-of-publication field should contain one value'
    );
    is_deeply(
        $docs->[0]->{'date-of-publication'}, ['1962'],
    ...
    is(
        scalar @{ $docs->[1]->{items_withdrawn_status} }, 1,
        'Second document items_withdrawn_status field should have one value'
    );
    is_deeply(
    ...
    is(
        $decoded_marc_record->as_xml_record(), $large_marc_record->as_xml_record(),
...
subtest 'Koha::SearchEngine::Elasticsearch::marc_records_to_documents_array () tests' => sub {
    ...
    is(
        $decoded_marc_record->as_usmarc(), $marc_record_1->as_usmarc(),
        "Decoded ARRAY record has same data as original record"
    );
};
...
subtest 'Koha::SearchEngine::Elasticsearch::marc_records_to_documents () authority tests' => sub {
    ...
    my $auth_type =
        $builder->build_object( { class => 'Koha::Authority::Types', value => { auth_tag_to_report => '150' } } );
    ...
    $marc_record_2->append_fields(
        MARC::Field->new(
            '150', '', '', a => 'Subject', v => 'Genresubdiv', z => 'Geosubdiv', x => 'Generalsubdiv',
            e => 'wrongsubdiv'
        ),
    );
    my $marc_record_3 = MARC::Record->new();
...
subtest 'Koha::SearchEngine::Elasticsearch::marc_records_to_documents with IncludeSeeFromInSearches' => sub {
    ...
    is_deeply(
        $docs->[0]->{subject__suggestion}, [ { input => 'Foo' } ],
        'subject__suggestion should not include "See from"'
    );
