...
$holds  = $patron->holds;
my $reserveid = Koha::Holds->search( { biblionumber => $biblio->biblionumber, borrowernumber => $borrowernumbers[0] } )
    ->next->reserve_id;
...
$hold->delete;
throws_ok { C4::Reserves::ModReserve( { reserve_id => $hold->reserve_id } ) }
...
my $foreign_item =
    $builder->build_sample_item( { library => $branch_2, biblionumber => $foreign_biblio->biblionumber } );
...
ok(
    CanItemBeReserved( $patrons[0], $foreign_item )->{status} eq 'cannot_reserve_from_other_branches',
...

{
    ...
    $itemnumber =
        $builder->build_sample_item( { library => $branch_1, biblionumber => $biblio->biblionumber } )->itemnumber;
    ...
    is(
        $hold3->discard_changes->priority, 1,
        "After ModReserve, the 3rd reserve becomes the first on the waiting list"
    );
}
...
is(
    CanItemBeReserved( $patrons[0], $damaged_item )->{status}, 'OK',
    "Patron can reserve damaged item with AllowHoldsOnDamagedItems enabled"
);
ok(
    defined( ( CheckReserves($damaged_item) )[1] ),
    "Hold can be trapped for damaged item with AllowHoldsOnDamagedItems enabled"
);
...
is(
    CanItemBeReserved( $patrons[0], $damaged_item )->{status},
...
ok(
    CanItemBeReserved( $patrons[0], $damaged_item )->{status} eq 'damaged',
    "Patron cannot reserve damaged item with AllowHoldsOnDamagedItems disabled"
);
ok(
    !defined( ( CheckReserves($damaged_item) )[1] ),
    "Hold cannot be trapped for damaged item with AllowHoldsOnDamagedItems disabled"
);
...
is(
    CanItemBeReserved( $patrons[0], $nfl_item )->{status}, 'OK',
    "Patron can place hold on item that is not for loan but holdable ( notforloan < 0 )"
);
$hold = Koha::Hold->new(
    {
        borrowernumber => $borrowernumbers[0],
...
ok(
    !defined( ( CheckReserves($nfl_item) )[1] ),
    "Hold cannot be trapped for item that is not for loan but holdable ( notforloan < 0 )"
);
t::lib::Mocks::mock_preference( 'TrapHoldsOnOrder', 1 );
ok(
    defined( ( CheckReserves($nfl_item) )[1] ),
    "Hold is trapped for item that is not for loan but holdable ( notforloan < 0 )"
);
t::lib::Mocks::mock_preference( 'SkipHoldTrapOnNotForLoanValue', '-1' );
ok(
    !defined( ( CheckReserves($nfl_item) )[1] ),
    "Hold cannot be trapped for item with notforloan value matching SkipHoldTrapOnNotForLoanValue"
);
...
is(
    CanItemBeReserved( $patrons[0], $nfl_item )->{status}, 'item_already_on_hold',
...
$item =
    $builder->build_sample_item( { library => $branch_1, itype => 'CANNOT', biblionumber => $biblio->biblionumber } );
AddReserve(
    {
        branchcode     => $branch_1,
        borrowernumber => $borrowernumbers[0],
...
is(
    CanItemBeReserved( $patrons[0], $item )->{status}, 'no_reserves_allowed',
    "cannot request item if policy that matches on item-level item type forbids it"
);
is(
    CanItemBeReserved( $patrons[0], $item, undef, { ignore_hold_counts => 1 } )->{status}, 'no_reserves_allowed',
...
subtest 'CanItemBeReserved' => sub {
    ...
    subtest 'no_reserves_allowed' => sub {
        plan tests => 5;
        ...
        my $biblionumber_record_cannot =
            $builder->build_sample_biblio( { itemtype => $itemtype_cant_record } )->biblionumber;

        my $item_1_can = $builder->build_sample_item(
            {
                homebranch   => $branch_1, holdingbranch => $branch_1, itype => $itemtype_can,
                biblionumber => $biblionumber_cannot
            }
        );
        my $item_1_cannot = $builder->build_sample_item(
            {
                homebranch   => $branch_1, holdingbranch => $branch_1, itype => $itemtype_cant,
                biblionumber => $biblionumber_cannot
            }
        );
        my $item_2_can = $builder->build_sample_item(
            {
                homebranch   => $branch_1, holdingbranch => $branch_1, itype => $itemtype_can,
                biblionumber => $biblionumber_can
            }
        );
        ...
        my $item_3_cannot = $builder->build_sample_item(
            {
                homebranch   => $branch_1, holdingbranch => $branch_1, itype => $itemtype_cant_record,
                biblionumber => $biblionumber_record_cannot
            }
        );
        ...
        t::lib::Mocks::mock_preference( 'item-level_itypes', 1 );
        is(
            CanItemBeReserved( $patrons[0], $item_2_cannot )->{status}, 'no_reserves_allowed',
        ...
        is(
            CanItemBeReserved( $patrons[0], $item_1_can )->{status}, 'no_reserves_allowed',
        ...
        is(
            CanItemBeReserved( $patrons[0], $item_3_cannot )->{status}, 'no_reserves_allowed',
            "When no holds allowed and no holds per record allowed should return no_reserves_allowed"
        );
    };

    subtest 'too_many_holds_for_this_record + too_many_reserves + item_already_on_hold' => sub {
        plan tests => 7;

        my $biblionumber_1 = $builder->build_sample_biblio( { itemtype => $itemtype_can } )->biblionumber;
        my $item_11        = $builder->build_sample_item(
            {
                homebranch   => $branch_1, holdingbranch => $branch_1, itype => $itemtype_can,
        ...
        my $item_22 = $builder->build_sample_item(
            {
                homebranch   => $branch_1, holdingbranch => $branch_1, itype => $itemtype_can,
                biblionumber => $biblionumber_2
            }
        ...
        is(
            CanItemBeReserved( $patrons[0], $item_12 )->{status}, 'too_many_holds_for_this_record',
        ...
        is(
            CanItemBeReserved( $patrons[0], $item_12 )->{status}, 'too_many_holds_for_this_record',
            "A item-level hold already exists and holds_per_record=1 - too_many_holds_for_this_record has priority over too_many_reserves"
        );
        Koha::CirculationRules->set_rules(
            {
                categorycode => undef,
        ...
        is(
            CanItemBeReserved( $patrons[0], $item_21 )->{status}, 'item_already_on_hold',
            "A item-level holds already exists on this item, item_already_on_hold should be raised"
        );
        is(
            CanItemBeReserved( $patrons[0], $item_22 )->{status}, 'too_many_reserves',
            "This patron has already placed reservesallowed holds, too_many_reserves should be raised"
        );
    };
};
...
my $branch_rule_item =
    $builder->build_sample_item( { library => $branch_1, itype => 'CANNOT', biblionumber => $biblio->biblionumber } );
is(
    CanItemBeReserved( $patrons[0], $branch_rule_item )->{status}, 'not_reservable',
    "CanItemBeReserved should return 'not_reservable'"
);

t::lib::Mocks::mock_preference( 'ReservesControlBranch', 'PatronLibrary' );
$branch_rule_item =
    $builder->build_sample_item( { library => $branch_2, itype => 'CAN', biblionumber => $biblio->biblionumber } );
is(
    CanItemBeReserved( $patrons[0], $branch_rule_item )->{status},
...
$branch_rule_item =
    $builder->build_sample_item( { library => $branch_1, itype => 'CAN', biblionumber => $biblio->biblionumber } );
is(
    CanItemBeReserved( $patrons[0], $branch_rule_item )->{status}, 'OK',
...
is(
    CanItemBeReserved( $patrons[0], $limit_count_item )->{status},
...
is(
    CanItemBeReserved( $patrons[0], $limit_count_item )->{status},
    'too_many_reserves', 'Patron cannot reserve item with hold limit of 1, 1 bib level hold placed'
);
is(
    CanItemBeReserved( $patrons[0], $limit_count_item, undef, { ignore_hold_counts => 1 } )->{status},
...
subtest 'Test max_holds per library/patron category' => sub {
    ...
    $ret = CanItemBeReserved( $patrons[0], $max_holds_item );
    is( $ret->{status}, 'too_many_reserves', 'Patron cannot place hold with only a category rule of 3' );
    ...
    $ret = CanItemBeReserved( $patrons[0], $max_holds_item );
    is( $ret->{status}, 'too_many_reserves', 'Patron cannot place hold with only a branch/category rule of 3' );
...
subtest 'Pickup location availability tests' => sub {
    ...
    $library->pickup_location('1')->store;
    is(
        CanItemBeReserved( $patron, $pickup_item, $branch_to )->{status},
    ...
    is(
        CanItemBeReserved( $patron, $pickup_item, $branch_to )->{status},
        'cannot_be_transferred', 'Item cannot be transferred'
    );
    $limit->delete;
    ...
    is(
        CanItemBeReserved( $patron, $pickup_item, $branch_to )->{status},
        'library_not_pickup_location', 'Library is not a pickup location'
    );
    is(
        CanItemBeReserved( $patron, $pickup_item, 'nonexistent' )->{status},
        'library_not_found', 'Cannot set unknown library as pickup location'
    );
};
...
subtest 'CanItemBeReserved / holds_per_day tests' => sub {
    ...
    my $item_1 =
        $builder->build_sample_item( { library => $library->branchcode, biblionumber => $biblio_1->biblionumber } );
    my $biblio_2 = $builder->build_sample_biblio( { itemtype => $itemtype->itemtype } );
    my $item_2 =
        $builder->build_sample_item( { library => $library->branchcode, biblionumber => $biblio_2->biblionumber } );
    my $biblio_3 = $builder->build_sample_biblio( { itemtype => $itemtype->itemtype } );
    my $item_3 =
        $builder->build_sample_item( { library => $library->branchcode, biblionumber => $biblio_3->biblionumber } );
    ...
    is_deeply(
        CanItemBeReserved( $patron, $item_1 ),
        { status => 'too_many_reserves', limit => 1 },
        'Patron cannot reserve item with hold limit of 1, 1 bib level hold placed'
    );

    # Raise reservesallowed to avoid too_many_reserves from it
    ...
    is_deeply(
        CanItemBeReserved( $patron, $item_2 ),
        { status => 'too_many_reserves_today', limit => 2 },
    ...
    is_deeply(
        CanItemBeReserved( $patron, $item_2 ),
        { status => 'too_many_reserves_today', limit => 0 },
    ...
    is_deeply(
        CanItemBeReserved( $patron, $item_3 ),
        { status => 'too_many_reserves', limit => 3 },
...
subtest 'CanItemBeReserved / branch_not_in_hold_group' => sub {
    ...
    my $patron3 =
        $builder->build_object( { class => 'Koha::Patrons', value => { branchcode => $library3->branchcode } } );
    ...
    is_deeply(
        CanItemBeReserved( $patron3, $item_2 ),
        { status => 'branch_not_in_hold_group' },
...
subtest 'CanItemBeReserved / pickup_not_in_hold_group' => sub {
    ...
    is_deeply(
        CanItemBeReserved( $patron3, $item_2, $library3->branchcode ),
        { status => 'pickup_not_in_hold_group' },
...
subtest 'non priority holds' => sub {
    ...
    my ( $ok, $err ) = CanBookBeRenewed( $patron1, $issue );
    ...
    ( $ok, $err ) = CanBookBeRenewed( $patron1, $issue );
...
subtest 'CanItemBeReserved / recall' => sub {
    ...
    is(
        CanItemBeReserved( $patron1, $item1, $library1->branchcode )->{status}, 'recall',
        "Can't reserve an item that they have already recalled"
    );
...
subtest 'ModReserve can only update expirationdate for found holds' => sub {
    ...
    my $biblio   = $builder->build_sample_biblio( { itemtype => 'DUMMY' } );
    my $itemnumber =
        $builder->build_sample_item( { library => $branch, biblionumber => $biblio->biblionumber } )->itemnumber;
    ...
    is(
        $hold->expirationdate, '1981-06-10',
...
subtest 'Koha::Holds->get_items_that_can_fill returns items with datecancelled or (inclusive) datearrived' => sub {
    ...
    my $transfer3 = $builder->build_object(
        {
            class => "Koha::Item::Transfers",
            value => {
                datecancelled => '2022-06-12',
    ...
    my $transfer4 = $builder->build_object(
        {
            class => "Koha::Item::Transfers",
            value => {
                datecancelled => undef,
    ...
    is(
        $items_that_can_fill1->next->id, $item1->id,
        "Koha::Holds->get_items_that_can_fill returns item with defined datearrived and datecancelled"
    );
    is(
        $items_that_can_fill1->count, 1,
        "Koha::Holds->get_items_that_can_fill returns 1 item with correct parameters"
    );
    is(
        $items_that_can_fill2->next->id, $item2->id,
        "Koha::Holds->get_items_that_can_fill returns item with defined datearrived and undefined datecancelled"
    );
    is(
        $items_that_can_fill2->count, 1,
        "Koha::Holds->get_items_that_can_fill returns 1 item with correct parameters"
    );
    is(
        $items_that_can_fill3->next->id, $item3->id,
        "Koha::Holds->get_items_that_can_fill returns item with undefined datearrived and defined datecancelled"
    );
    is(
        $items_that_can_fill3->count, 1,
        "Koha::Holds->get_items_that_can_fill returns 1 item with correct parameters"
    );
    is(
        $items_that_can_fill4->next, undef,
        "Koha::Holds->get_items_that_can_fill doesn't return item with undefined datearrived and undefined datecancelled"
    );
