...
sub get_privileged_user {
    ...
    return ( $librarian->userid, $librarian->borrowernumber, $password );
}
...
subtest 'list()' => sub {
    ...
    $t->get_ok("//$u:$p@/api/v1/item-lists")->status_is(200)->json_is( [ $list->to_api() ] );
...
subtest 'get()' => sub {
    ...
    $t->get_ok("//$u:$p@/api/v1/item-lists/$list_id")->status_is(200)->json_is( $list->to_api );

    $t->get_ok("//$u:$p@/api/v1/item-lists/$list2_id")->status_is(200)->json_is( $list2->to_api );
...
subtest 'add_share()' => sub {
    ...
    $t->post_ok( "/api/v1/item-lists/$list_id/shares" => json => { patron_id => $patron->id, permission => 'view' } )
        ->status_is(401);
    ...
    $t->post_ok(
        ...
        ->status_is(400);
    $t->post_ok( "//$u:$p@/api/v1/item-lists/$list_id/shares" => json => { patron_id => 9999, permission => 'view' } )
        ->status_is(404);
    $t->post_ok(
        "//$u:$p@/api/v1/item-lists/9999/shares" => json => { patron_id => $patron->id, permission => 'view' } )
        ->status_is(404);
