...
subtest 'list() tests' => sub {
    ...
    $fund->delete;
    $another_fund->delete;
    $t->get_ok(qq~//$userid:$password@/api/v1/acquisitions/funds?q=[{"me.code":{"like":"%ko%"}}]~)
        ->status_is(200)
        ->json_is( [] );
    ...
    $t->get_ok("//$userid:$password@/api/v1/acquisitions/funds?blah=blah")
        ->status_is(400)
...
subtest 'get() tests' => sub {
    ...
    my $non_existent_funds = $builder->build_object(
        {
            class => 'Koha::Acquisition::Finances::Funds',
            value => { lib_group_visibility => "|" . $lib_group->id . "|" }
        }
...
subtest 'add() tests' => sub {
    ...
    my $fund_id =
        ...
        ->json_is( '/description'          => $fund->{description} )
        ->json_is( '/code'                 => $fund->{code} )
        ->json_is( '/status'               => $fund->{status} )
        ->json_is( '/owner_id'             => $ledger->owner_id )
        ->json_is( '/lib_group_visibility' => $fund->{lib_group_visibility} )
        ->tx->res->json->{fund_id};
...
subtest 'update() tests' => sub {
    ...
    my $funds_to_delete = $builder->build_object(
        {
            class => 'Koha::Acquisition::Finances::Funds',
            value => { lib_group_visibility => "|" . $lib_group->id . "|" }
        }
