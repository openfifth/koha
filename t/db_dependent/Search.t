...
$contextmodule->mock(
    ...
    sub {
        ...
        if ( $pref eq 'marcflavour' ) {
            ...
            return C4::XSLT::_get_best_default_xslt_filename(
                $htdocs, 'bootstrap', 'en',
                $marcflavour . 'slim2OPACResults.xsl'
            );
...
sub run_marc21_search_tests {
    ...
    my %itemtypes = (
        ...
        'CF' => {
            'imageurl'    => 'bridge/computer_file.png', 'summary' => '', 'itemtype' => 'CF',
            'description' => 'Computer Files'
        },
        'CR' => {
            'imageurl'    => 'bridge/periodical.png', 'summary' => '', 'itemtype' => 'CR',
        ...
        'MX' =>
            { 'imageurl' => 'bridge/kit.png', 'summary' => '', 'itemtype' => 'MX', 'description' => 'Mixed Materials' },
        'REF' => { 'imageurl' => '', 'summary' => '', 'itemtype' => 'REF', 'description' => 'Reference' },
        'VM'  => {
            'imageurl' => 'bridge/dvd.png', 'summary' => '', 'itemtype' => 'VM', 'description' => 'Visual Materials'
        },
    );
    ...
    is(
        $total_hits, 1,
        "SimpleSearch on music publisher number works using Identifier-publisher-for-music (bug 8252)"
    );
    ...
    is(
        $results_hashref->{biblioserver}->{hits}, 101,
        "getRecords keyword search for 'book' matched right number of records"
    );
    is(
        scalar @{ $results_hashref->{biblioserver}->{RECORDS} }, 19,
        "getRecords returned requested number of records"
    );
    ...
    is(
        $results_hashref->{biblioserver}->{hits}, 11,
        "getRecords title search for 'book' matched right number of records"
    );

    ( undef, $results_hashref, $facets_loop ) =
        getRecords( 'au:Lessig', 'au:Lessig', [], ['biblioserver'], '20', 0, \%branches, \%itemtypes, 'ccl', undef );
    is(
        $results_hashref->{biblioserver}->{hits}, 4,
        "getRecords title search for 'Australia' matched right number of records"
    );

    ( undef, $results_hashref, $facets_loop ) =
        getRecords( 'salud', 'salud', [], ['biblioserver'], '19', 0, \%branches, \%itemtypes, 'ccl', undef );
    ok(
        MARC::Record::new_from_xml( $results_hashref->{biblioserver}->{RECORDS}->[0], 'UTF-8' )->title_proper() =~
            m/^Efectos del ambiente/
            && MARC::Record::new_from_xml( $results_hashref->{biblioserver}->{RECORDS}->[7], 'UTF-8' )->title_proper()
            eq 'Salud y seguridad de los trabajadores del sector salud: manual para gerentes y administradores^ies'
            && MARC::Record::new_from_xml( $results_hashref->{biblioserver}->{RECORDS}->[18], 'UTF-8' )->title_proper()
            =~ m/^Indicadores de resultados identificados/,
    ...
    ok(
        MARC::Record::new_from_xml( $results_hashref->{biblioserver}->{RECORDS}->[0], 'UTF-8' )->title_proper() =~
            m/la enfermedad laboral\^ies$/
            && MARC::Record::new_from_xml( $results_hashref->{biblioserver}->{RECORDS}->[6], 'UTF-8' )->title_proper()
            =~ m/^Indicadores de resultados identificados/
            && MARC::Record::new_from_xml( $results_hashref->{biblioserver}->{RECORDS}->[18], 'UTF-8' )->title_proper()
            eq 'World health statistics 2009^ien',
    ...
    ok( MARC::Record::new_from_xml( $results_hashref->{biblioserver}->{RECORDS}->[0], 'UTF-8' )->title_proper() eq
            'World health statistics 2009^ien'
            && MARC::Record::new_from_xml( $results_hashref->{biblioserver}->{RECORDS}->[12], 'UTF-8' )->title_proper()
            =~ m/^Indicadores de resultados identificados/
            && MARC::Record::new_from_xml( $results_hashref->{biblioserver}->{RECORDS}->[18], 'UTF-8' )->title_proper()
            =~ m/la enfermedad laboral\^ies$/, "Simple descending author sorting in getRecords matches old behavior" );

    ( undef, $results_hashref, $facets_loop ) = getRecords(
        'salud', 'salud', ['pubdate_asc'], ['biblioserver'], '38', 0, \%branches, \%itemtypes, 'ccl',
        undef
    );
    ok(
        MARC::Record::new_from_xml( $results_hashref->{biblioserver}->{RECORDS}->[0], 'UTF-8' )->title_proper() eq
            'Manual de higiene industrial^ies'
            && MARC::Record::new_from_xml( $results_hashref->{biblioserver}->{RECORDS}->[7], 'UTF-8' )->title_proper()
            =~ m/seguridad e higiene del trabajo\^ies$/
            && MARC::Record::new_from_xml( $results_hashref->{biblioserver}->{RECORDS}->[18], 'UTF-8' )->title_proper()
            =~ m/^Indicadores de resultados identificados/,
    ...
    ( undef, $results_hashref, $facets_loop ) = getRecords(
        'salud', 'salud', ['pubdate_dsc'], ['biblioserver'], '38', 0, \%branches, \%itemtypes, 'ccl',
        undef
    );
    ok(
        MARC::Record::new_from_xml( $results_hashref->{biblioserver}->{RECORDS}->[0], 'UTF-8' )->title_proper() =~
            ...
            eq 'World health statistics 2009^ien'
            && MARC::Record::new_from_xml( $results_hashref->{biblioserver}->{RECORDS}->[18], 'UTF-8' )->title_proper()
            eq 'Manual de higiene industrial^ies',
    ...
    (
        ...
        $query_desc, $limit, $limit_cgi,    $limit_desc,
        $query_type
    ) = buildQuery( [], ['salud'], [], [], [], 0, 'en' );
    like( $query, qr/kw\W.*salud/, "Built CCL keyword query" );

    ( $error, $results_hashref, $facets_loop ) =
        getRecords( $query, $simple_query, [], ['biblioserver'], 20, 0, \%branches, \%itemtypes, $query_type, 0 );
    is(
        $results_hashref->{biblioserver}->{hits}, 19,
        "getRecords generated keyword search for 'salud' matched right number of records"
    );

    my @newresults = searchResults(
        { 'interface' => 'opac' }, $query_desc, $results_hashref->{'biblioserver'}->{'hits'}, 18, 0, 0,
    ...
    (
        ...
        $query_desc, $limit, $limit_cgi,    $limit_desc,
        $query_type
    ) = buildQuery( ['AND'], [ 'salud', 'higiene' ], [], [], [], 0, 'en' );
    like( $query, qr/kw\W.*salud\W.*AND.*kw\W.*higiene/, "Built composed explicit-and CCL keyword query" );

    ( $error, $results_hashref, $facets_loop ) =
        getRecords( $query, $simple_query, [], ['biblioserver'], 20, 0, \%branches, \%itemtypes, $query_type, 0 );
    is(
        $results_hashref->{biblioserver}->{hits}, 3,
        "getRecords generated composed keyword search for 'salud' explicit-and 'higiene' matched right number of records"
    );
    ...
    (
        ...
        $query_desc, $limit, $limit_cgi,    $limit_desc,
        $query_type
    ) = buildQuery( ['OR'], [ 'salud', 'higiene' ], [], [], [], 0, 'en' );
    like( $query, qr/kw\W.*salud\W.*OR.*kw\W.*higiene/, "Built composed explicit-or CCL keyword query" );

    ( $error, $results_hashref, $facets_loop ) =
        getRecords( $query, $simple_query, [], ['biblioserver'], 20, 0, \%branches, \%itemtypes, $query_type, 0 );
    is(
        $results_hashref->{biblioserver}->{hits}, 20,
        "getRecords generated composed keyword search for 'salud' explicit-or 'higiene' matched right number of records"
    );
    ...
    is(
        $results_hashref->{biblioserver}->{hits}, 3,
        "getRecords generated composed keyword search for 'salud' implicit-and 'higiene' matched right number of records"
    );
    ...
    (
        ...
        $query_desc, $limit, $limit_cgi,    $limit_desc,
        $query_type
    ) = buildQuery( [], ['salud'], ['kw'], ['su-to:Laboratorios'], [], 0, 'en' );
    ...
    ( $error, $results_hashref, $facets_loop ) =
        getRecords( $query, $simple_query, [], ['biblioserver'], 20, 0, \%branches, \%itemtypes, $query_type, 0 );
    is(
        $results_hashref->{biblioserver}->{hits}, 2,
        "getRecords generated faceted search matched right number of records"
    );
    ...
    (
        ...
        $query_desc, $limit, $limit_cgi,    $limit_desc,
        $query_type
    ) = buildQuery( [], [''], ['kw'], [ 'mc-itype:MP', 'mc-itype:MU' ], [], 0, 'en' );

    ( $error, $results_hashref, $facets_loop ) =
        getRecords( $query, $simple_query, [], ['biblioserver'], 20, 0, \%branches, \%itemtypes, $query_type, 0 );
    is(
        $results_hashref->{biblioserver}->{hits}, 2,
        "getRecords generated mc-faceted search matched right number of records"
    );
    ...
    (
        ...
        $query_desc, $limit, $limit_cgi,    $limit_desc,
        $query_type
    ) = buildQuery( [], [''], ['kw'], [ 'mc-loc:GEN', 'branch:FFL' ], [], 0, 'en' );

    ( $error, $results_hashref, $facets_loop ) =
        getRecords( $query, $simple_query, [], ['biblioserver'], 20, 0, \%branches, \%itemtypes, $query_type, 0 );
    is(
        $results_hashref->{biblioserver}->{hits}, 2,
        "getRecords generated multi-faceted search matched right number of records"
    );
    ...
    (
        ...
        $query_desc, $limit, $limit_cgi,    $limit_desc,
        $query_type
    ) = buildQuery( [], ['NEKLS'], ['Code-institution'], [], [], 0, 'en' );
    ( $error, $results_hashref, $facets_loop ) =
        getRecords( $query, $simple_query, [], ['biblioserver'], 20, 0, \%branches, \%itemtypes, $query_type, 0 );
    is(
        $results_hashref->{biblioserver}->{hits}, 12,
    ...
    (
        ...
        $query_desc, $limit, $limit_cgi,    $limit_desc,
        $query_type
    ) = buildQuery( [], [''], ['kw'], ['available'], [], 0, 'en' );

    ( $error, $results_hashref, $facets_loop ) =
        getRecords( $query, $simple_query, [], ['biblioserver'], 20, 0, \%branches, \%itemtypes, $query_type, 0 );
    is(
        $results_hashref->{biblioserver}->{hits}, 2,
        "getRecords generated availability-limited search matched right number of records"
    );

    {
        ...
        @newresults = searchResults(
            { 'interface' => 'opac' }, $query_desc, $results_hashref->{'biblioserver'}->{'hits'}, 17, 0, 0,
            $results_hashref->{'biblioserver'}->{"RECORDS"}
        );
    ...
    like(
        $newresults[0]->{XSLTResultsRecord}, qr/<variable name="anonymous_session">1<\/variable>/,
        "Variable injected correctly"
    );
    ...
    unlike(
        $newresults[0]->{XSLTResultsRecord}, qr/<subfield code="p">TEST11111<\/subfield>/,
        '952\$p hidden in OPAC'
    );
    ...
    unlike(
        $newresults[0]->{XSLTResultsRecord}, qr/<subfield code="y">Books<\/subfield>/,
        '952\$y hidden on staff interface'
    );
    ...
    (
        ...
        $query_desc, $limit, $limit_cgi,    $limit_desc,
        $query_type
    ) = buildQuery( [], ['pqf=@attr 1=_ALLRECORDS @attr 2=103 ""'], [], [], [], 0, 'en' );
    ...
    (
        ...
        $query_desc, $limit, $limit_cgi,    $limit_desc,
        $query_type
    ) = buildQuery( [], ['pqf=@attr 1=1016 "Lessig"'], [], [], [], 0, 'en' );

    ( $error, $results_hashref, $facets_loop ) =
        getRecords( $query, $simple_query, [], ['biblioserver'], 20, 0, \%branches, \%itemtypes, $query_type, 0 );
    is(
        $results_hashref->{biblioserver}->{hits}, 4,
        "getRecords PQF author search for Lessig returned proper number of matches"
    );
    ...
    (
        ...
        $query_desc, $limit, $limit_cgi,    $limit_desc,
        $query_type
    ) = buildQuery( [], ['ccl=au:Lessig'], [], [], [], 0, 'en' );

    ( $error, $results_hashref, $facets_loop ) =
        getRecords( $query, $simple_query, [], ['biblioserver'], 20, 0, \%branches, \%itemtypes, $query_type, 0 );
    is(
        $results_hashref->{biblioserver}->{hits}, 4,
        "getRecords CCL author search for Lessig returned proper number of matches"
    );
    ...
    (
        ...
        $query_desc, $limit, $limit_cgi,    $limit_desc,
        $query_type
    ) = buildQuery( [], ['cql=dc.author any lessig'], [], [], [], 0, 'en' );

    ( $error, $results_hashref, $facets_loop ) =
        getRecords( $query, $simple_query, [], ['biblioserver'], 20, 0, \%branches, \%itemtypes, $query_type, 0 );
    is(
        $results_hashref->{biblioserver}->{hits}, 4,
        "getRecords CQL author search for Lessig returned proper number of matches"
    );
    ...
    is(
        MARC::Record::new_from_xml( $results_hashref->{biblioserver}->{RECORDS}->[0], 'UTF-8' )->title_proper(),
        'Salud y seguridad de los trabajadores del sector salud: manual para gerentes y administradores^ies',
        "Weighted query returns best match first"
    );
    ...
    (
        ...
        $query_desc, $limit, $limit_cgi,    $limit_desc,
        $query_type
    ) = buildQuery( [], ['medic'], ['kw'], [], [], 0, 'en' );

    ( $error, $results_hashref, $facets_loop ) =
        getRecords( $query, $simple_query, [], ['biblioserver'], 20, 0, \%branches, \%itemtypes, $query_type, 0 );
    is(
        $results_hashref->{biblioserver}->{hits}, 5,
        "Search for 'medic' returns matches  with automatic truncation on"
    );
    ...
    (
        ...
        $query_desc, $limit, $limit_cgi,    $limit_desc,
        $query_type
    ) = buildQuery( [], ['web application'], ['kw'], [], [], 0, 'en' );
    ( $error, $results_hashref, $facets_loop ) =
        getRecords( $query, $simple_query, [], ['biblioserver'], 20, 0, \%branches, \%itemtypes, $query_type, 0 );
    is(
        $results_hashref->{biblioserver}->{hits}, 1,
        "Search for 'web application' returns one hit with QueryWeightFields on"
    );
    ...
    is(
        $results_hashref->{biblioserver}->{hits}, 1,
        "Search for 'web \"application' returns one hit with QueryWeightFields on (bug 7518)"
    );
    ...
    ( $error, $results_hashref, $facets_loop ) =
        getRecords( $query, $simple_query, [], ['biblioserver'], 20, 0, \%branches, \%itemtypes, $query_type, 0 );
    is(
        $results_hashref->{biblioserver}->{hits}, undef,
        "Search for 'medic' returns no matches with automatic truncation off"
    );
    ...
    is(
        $results_hashref->{biblioserver}->{hits}, 5,
        "Search for 'medic*' returns matches with automatic truncation off"
    );
    ...
    (
        ...
        $query_desc, $limit, $limit_cgi,    $limit_desc,
        $query_type
    ) = buildQuery( [], ['pressed'], ['kw'], [], [], 0, 'en' );

    ( $error, $results_hashref, $facets_loop ) =
        getRecords( $query, $simple_query, [], ['biblioserver'], 20, 0, \%branches, \%itemtypes, $query_type, 0 );
    is(
        $results_hashref->{biblioserver}->{hits}, 7,
        "Search for 'pressed' returns matches when stemming (and query weighting) is on"
    );
    ...
    is(
        $results_hashref->{biblioserver}->{hits}, undef,
        "Search for 'pressed' returns no matches when stemming is off"
    );
    ...
    (
        ...
        $query_desc, $limit, $limit_cgi,    $limit_desc,
        $query_type
    ) = buildQuery( [], ['ccl=an:42'], [], ['available'], [], 0, 'en' );
    is(
        $query,
        "an:42 and (( (allrecords,AlwaysMatches='') and (not-onloan-count,st-numeric >= 1) and (lost,st-numeric=0) ))",
        'buildQuery should add the available part to the query if requested with ccl'
    );
    ...
    (
        ...
        $query_desc, $limit, $limit_cgi,    $limit_desc,
        $query_type
    ) = buildQuery( [], ['ccl=an:42'], [], ['branch:CPL'], [], 0, 'en' );
    is(
        $query, "an:42 and (homebranch: CPL or holdingbranch: CPL)",
        'buildQuery should expand the limit as necessary for ccl queries'
    );
    ...
    (
        ...
        $query_desc, $limit, $limit_cgi,    $limit_desc,
        $query_type
    ) = buildQuery( [], [], [], ['mc-ccode:NF(IC'], [], 0, 'en' );
    ...
    warning_like {
        ( undef, $results_hashref, $facets_loop ) = getRecords(
            'pqf=@attr 1=4 "title"', 'pqf=@attr 1=4 "title"', [], ['biblioserver'], '19', 0, \%branches,
            \%itemtypes, '', undef
        )
    }
    ...
    ( $error, $results_hashref, $facets_loop ) = getRecords(
        "Godzina pąsowej róży", "Godzina pąsowej róży", [], ['biblioserver'], 20, 0, \%branches,
        \%itemtypes,            $query_type,            0
    );
    @newresults = searchResults(
        { 'interface' => 'intranet' }, $query_desc, $results_hashref->{'biblioserver'}->{'hits'}, 17, 0, 0,
        $results_hashref->{'biblioserver'}->{"RECORDS"}
    );
    ...
    ( undef, $results_hashref, $facets_loop ) = getRecords(
        'ti:marc the large record', '', [], ['biblioserver'], '20', 0, \%branches, \%itemtypes, 'ccl',
        undef
    );
    is(
        $results_hashref->{biblioserver}->{hits}, 1,
        "Can do a search that retrieves an over-large bib record (bug 11096)"
    );

    {
        ...
        $mock_items->mock( 'count', 1 );
        @newresults = searchResults(
            { 'interface' => 'opac' }, $query_desc, $results_hashref->{'biblioserver'}->{'hits'}, 10, 0, 0,
            $results_hashref->{'biblioserver'}->{"RECORDS"}
        );
        is(
            $newresults[0]->{title}, 'Marc the Large Record',
            'Able to render the title for over-large bib record (bug 11096)'
        );
        is( $newresults[0]->{biblionumber}, '300', 'Over-large bib record has the correct biblionumber (bug 11096)' );
        like(
            $newresults[0]->{notes}, qr/This is large note #550/,
            'Able to render the notes field for over-large bib record (bug 11096)'
        );
    }

    # notforloancount should be returned as part of searchResults output
    ok(
        defined $newresults[0]->{notforloancount},
        '\'notforloancount\' defined in searchResults output (Bug 12419)'
    );
    is(
        $newresults[0]->{notforloancount}, 2,
    ...
    warning_like {
        ( undef, $results_hashref, $facets_loop ) = getRecords(
            'ccl=( AND )', '', ['title_az'], ['biblioserver'], '20', 0, \%branches, \%itemtypes, 'ccl',
            undef
        )
    }
    ...
    is_deeply(
        { au => { 'Cohen Arazi, Tomas' => 1 } }, $facets_counter,
    ...
    C4::Search::_get_facets_data_from_record( $marc_record, $facets, $facets_counter );
    is_deeply(
        { au => { 'Cohen Arazi, Tomas' => 2 } }, $facets_counter,
    ...
    is_deeply(
        $facets_info, $expected_facets_info_marc21,
...
sub run_unimarc_search_tests {
    ...
    my ( $error, $marcresults, $total_hits ) =
        SimpleSearch( "ti=Järnvägarnas efterfrågan och den svenska industrin", 0, 10 );
    ...
    delete $expected_facets_info_unimarc->{holdingbranch}
        if Koha::Libraries->count == 1;
    is_deeply(
        $facets_info, $expected_facets_info_unimarc,
        "_get_facets_info returns the correct data"
    );
...
subtest 'FindDuplicate' => sub {
    ...
    $record_2->add_fields( [ '245', '0', '0', a => 'The book of nothing /' ] );
    my $record_3 = MARC::Record->new;
    $record_3->add_fields( [ '245', '0', '0', a => 'Frog and toad all year /' ] );
