...
# Mock SP methods
{
    ...
    *Net::SAML2::SP::authn_request = sub {
        my ( $self, %args ) = @_;
        return bless { xml => '<AuthnRequest/>', %args }, 'Net::SAML2::AuthnRequest';
    };
    *Net::SAML2::SP::metadata = sub {
        return '<EntityDescriptor entityID="' . $sp_entity_id . '"><SPSSODescriptor/></EntityDescriptor>';
    };
    *Net::SAML2::SP::sign_redirect = sub { return 'mock_signature' };
}

{
    no warnings 'redefine';
    *Net::SAML2::AuthnRequest::as_xml = sub { return '<AuthnRequest/>' };
}
...
subtest '_init_sp() - missing required fields' => sub {
    ...
    # Missing both idp_metadata forms
    {
        my %bad_config = %$config_inline;
        delete $bad_config{idp_metadata};
        delete $bad_config{idp_metadata_path};
        eval { Koha::Auth::SAML2->new( \%bad_config ) };
        like( $@, qr/idp_metadata/, 'dies when idp_metadata and idp_metadata_path are both missing' );
    }
};
