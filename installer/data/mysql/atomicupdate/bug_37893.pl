...
return {
    ...
    up          => sub {
        ...
        foreach my $institution_key (@institution_keys) {
            ...
            my $offline        = $SIPconfig->{institutions}->{$institution_key}->{policy}->{offline}       // 'false';
            ...
            my $status_update  = $SIPconfig->{institutions}->{$institution_key}->{policy}->{status_update} // 'false';
