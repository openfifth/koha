...
return {
    ...
    up          => sub {
        ...
        unless ( column_exists( 'illrequests', 'managedby' ) ) {
            ...
            $dbh->do(q{ALTER TABLE reserves ADD KEY illrequests_manfk (managedby)});
            $dbh->do(
                q{ALTER TABLE reserves ADD CONSTRAINT illrequests_manfk FOREIGN KEY (managedby) REFERENCES borrowers (borrowernumber) ON DELETE SET NULL ON UPDATE CASCADE}
            );
