...
if ( $op eq 'cud-delete-order' ) {
    ...
    print GetBasketAsCSV( scalar $query->param('basketno'), $query, $csv_profile_id )
        ;    # if no csv_profile_id passed, using default rows
    exit;
} elsif ( $op eq 'cud-email' ) {
    my $err = eval { SendAlerts( 'orderacquisition', scalar $query->param('basketno'), 'ACQORDER' ); };
    if ($@) {
...
sub get_order_infos {
    ...
    if ($biblionumber) {    # The biblio still exists
        ...
        my $order             = Koha::Acquisition::Orders->find($ordernumber)
            ;    # FIXME We should certainly do that at the beginning of this sub
        my $items   = $order->items;
        my $invoice = $order->invoice;

        my $itemholds =
            $biblio->holds->search( { itemnumber => { -in => [ $items->get_column('itemnumber') ] } } )->count;
