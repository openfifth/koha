...
use C4::Acquisition
    qw( CloseInvoice ReopenInvoice ModInvoice MergeInvoices DelInvoice GetInvoice GetInvoiceDetails get_rounded_price );
...
if ( C4::Context->preference('AcqEnableFiles') ) {
    $invoice_files = Koha::Misc::Files->new( tabletag => 'aqinvoices', recordid => $invoiceid );
}
...
if ( $op && $op eq 'cud-close' ) {
    ...
    for ( my $i = 0 ; $i < scalar @adjustment ; $i++ ) {
        if ( $adjustment_id[$i] eq 'new' ) {
            ...
            unless ( $old_adj->adjustment == $adjustment[$i]
                && $old_adj->reason eq $reason[$i]
                && $old_adj->budget_id == $budget_id[$i]
                && $old_adj->encumber_open == $e_open{ $adjustment_id[$i] }
                && $old_adj->note eq $note[$i] )
            {
