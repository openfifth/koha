...
sub GetAuthMARCFromKohaField {
    ...
    my $sth = $dbh->prepare(
        "select tagfield,tagsubfield from auth_subfield_structure where kohafield= ? and authtypecode=? ");
    ...
    for ( my $i = 0 ; $i <= $#{$value} ; $i++ ) {
        if ( @$value[$i] ) {
            ...
            if ( $operator and $operator eq 'is' ) {
                ...
                $attr .=
                    " \@attr 4=1  \@attr 5=100 \@attr 6=3 ";    ##Phrase, No truncation,all of subfield field must match
            } else {
...
sub GuessAuthTypeCode {
    ...
    foreach my $field ( keys %{ $heading_fields->{ uc( C4::Context->preference('marcflavour') ) } } ) {
        return $heading_fields->{ uc( C4::Context->preference('marcflavour') ) }->{$field}->{'authtypecode'}
...
sub AddAuthority {
    ...
    if ( $format eq "MARC21" ) {
        ...
        if ( !$record->field('008') ) {
            $record->insert_fields_ordered( MARC::Field->new( '008', $date . Koha::Authority->default_marc21_008 ) );
        }
        if ( !$record->field('040') ) {
            $record->insert_fields_ordered(
                MARC::Field->new(
...
sub FindDuplicateAuthority {
    ...
    if ( !defined $error && @{$results} ) {
        ...
        return $marcrecord->field('001')->data,
            BuildSummary( $marcrecord, $marcrecord->field('001')->data, $authtypecode );
    }
...
sub BuildSummary {
    ...
    if ( C4::Context->preference('marcflavour') eq 'UNIMARC' ) {
        ...
        foreach my $field ( $record->field('880') ) {
            ...
            push @otherscript,
                {
                term      => $field->as_string($subfields_to_report), category => $category, type => $type,
    ...
    if ( $data->{'authtrees'} and not $force ) {
        ...
        foreach my $field ( $record->field('5..') ) {
            ...
            $broader = 1
                ...
                || (   C4::Context->preference('marcflavour') ne 'UNIMARC'
                    && $field->subfield('w')
                    && substr( $field->subfield('w'), 0, 1 ) eq 'g' )
                );
            if ($broader) {
...
sub merge {
    ...
    my $tags_using_authtype =
          $authtypefrom && $authtypefrom->authtypecode
        ? $dbh->selectcol_arrayref( $sql, undef, ( $authtypefrom->authtypecode ) )
        : $dbh->selectcol_arrayref(
        "SELECT DISTINCT tagfield FROM marc_subfield_structure WHERE authtypecode IS NOT NULL AND authtypecode<>''");
    my $tags_new;
