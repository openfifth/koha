...
sub CanItemBeReserved {
    ...
    my $count_individually = $count_policy && $count_policy->rule_value eq 'count_individually';
    my $skip_hold_count    = $params->{ignore_hold_counts}
        || ( $params->{hold_group_count_checked} && !$count_individually );
    ...
    if ( defined $allowedreserves && $allowedreserves ne '' ) {
        ...
        if ( !$skip_hold_count ) {
            ...
            my $select =
                ...
                : q{SELECT COUNT(CASE WHEN hold_group_id IS NULL THEN 1 END)
                         + COUNT(DISTINCT CASE WHEN hold_group_id IS NOT NULL THEN hold_group_id END) AS count };
            my $sth = $dbh->prepare( $select . $base_query );
            $sth->execute(@bind_params);
            my $reservecount = ( $sth->fetchrow_hashref // {} )->{count} // 0;
