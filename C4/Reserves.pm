...
sub CanBookBeReserved {
    ...
    if ( $params->{itemtype} && !$params->{ignore_group_hold_counts} ) {
        ...
        my $count =
            ( $count_policy && $count_policy->rule_value eq 'count_as_group' )
            ? $patron->holds->count_holds( $hold_search_params, { join => ['item'] } )
            : $patron->holds->search( $hold_search_params, { join => ['item'] } )->count();
...
=head2 CanItemBeReserved
  ...
  current params are:
  'ignore_group_hold_counts' - we use this routine to check if an item can fill a hold - on this case we
...
sub CanItemBeReserved {
    ...
    return _cache { status => 'itemAlreadyOnHold' }
        if ( !$params->{ignore_group_hold_counts}
    ...
    if ( defined $holds_per_record && $holds_per_record ne '' ) {
        ...
        if ( !$params->{ignore_group_hold_counts} ) {
            ...
            my $holds_count =
                $count_as_group
                ? Koha::Holds->count_holds($search_params)
                : Koha::Holds->search($search_params)->count();
            return _cache { status => "tooManyHoldsForThisRecord", limit => $holds_per_record }
                if $holds_count >= $holds_per_record;
        }
    }

    if ( !$params->{ignore_group_hold_counts} && defined $holds_per_day && $holds_per_day ne '' ) {
        my $date_search_params = {
            borrowernumber => $patron->borrowernumber,
            reservedate    => dt_from_string->date
        };
        my $today_holds_count =
            $count_as_group
            ? Koha::Holds->count_holds($date_search_params)
            : Koha::Holds->search($date_search_params)->count();
    ...
    if ( !$params->{ignore_group_hold_counts} && $rule && defined( $rule->rule_value ) && $rule->rule_value ne '' ) {
        my $total_holds_count =
            $count_as_group
            ? Koha::Holds->count_holds( { borrowernumber => $patron->borrowernumber } )
            : Koha::Holds->search( { borrowernumber => $patron->borrowernumber } )->count();
