...
sub ModWeekdayholiday {
    ...
    my %options = @_;
    my $closure =
        Koha::Library::Calendar::WeeklyClosures->search( { library_id => $self->{branchcode}, weekday => $options{weekday} } )
        ->next;
...
sub ModSingleholiday {
    ...
    my $closure = Koha::Library::Calendar::SingleClosures->search( { library_id => $self->{branchcode}, date => $date } )->next;
    if ($closure) {
...
sub ModExceptionholiday {
    ...
    my %options   = @_;
    my $date      = $options{date} || sprintf( ISO_DATE_FORMAT, $options{year}, $options{month}, $options{day} );
    my $exception = Koha::Library::Calendar::Exceptions->search( { library_id => $self->{branchcode}, date => $date } )->next;
    if ($exception) {
...
sub delete_holiday {
    ...
    if ( $options{day} && $options{month} && $options{year} ) {
        ...
        if ( Koha::Library::Calendar::SingleClosures->search( { library_id => $self->{branchcode}, date => $date } )->count ) {
