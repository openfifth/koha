$(document).ready(function () {
    ...
    function _addSuccessMessage(auto_backend_name, data) {
        _removeVerifyingMessage(auto_backend_name);
        $(auto_ill_el + " > #backend-" + auto_backend_name + " #backendcol").append(
            '<div class="text-success mb-2"><i class="fa-solid fa-check"></i> ' +
                "<strong>" + __("Available.") + "</strong><br>" + data.success +
                "</div>"
        );
    }
    ...
    function _addErrorMessage(auto_backend_name, message) {
        ...
        $(auto_ill_el + " > #backend-" + auto_backend_name + " #backendcol").append(
            '<div class="text-danger mb-2"> <i class="fa-solid fa-xmark"></i> ' +
                "<strong>" + __("Not readily available:") + "</strong><br>" +
                message +
                "</div>"
        );
        $(auto_ill_el + " > #backend-" + auto_backend_name + " input[type=\"radio\"]").prop("disabled", true);
    }
    ...
    $('#confirmautoill-form').on('change', 'input[name="backend"]', function() {
        $('input[name="backend"]').closest('.list-group-item').removeClass('bg-success').css({
            'background-color': '',
            'border': ''
        });
