...
function readCookieValue(str, val_beg) {
    ...
    if (val_end == -1) val_end = str.length;
...
function delCookie(name) {
    ...
    if (parent.opener) {
        parent.opener.document.cookie =
            name + "=null; SameSite=Lax; path=/; expires=" + exp.toGMTString();
    } else {
        document.cookie =
            name + "=null; SameSite=Lax; path=/; expires=" + exp.toGMTString();
    }
...
function SelectAll() {
    if (document.bookbag_form.biblionumber.length > 0) {
        ...
        for (var i = 0; i < l; i++) {
            document.bookbag_form.biblionumber[i].checked = checky
                ? false
                : true;
        }
    }
    ...
    if (biblist && biblist.length > 0) {
        ...
        if (bibnums.length > 0) {
            ...
            if (document.bookbag_form.biblionumber.length > 0) {
                for (
                    i = 0;
                    i < document.bookbag_form.biblionumber.length;
                    i++
                ) {
                ...
                c_value =
                    c_value + document.bookbag_form.biblionumber.value + "/";
            }
        }
    }
...
function addSelRecords(valSel) {
    ...
    if (nbAdd) {
        ...
        if (i < 1) {
            ...
            msg = __(
                "No record was added to your cart (already in your cart)!"
            );
        }
    }
    showCartUpdate(msg);
...
function quit() {
    if (document.myform.records.value) {
        var rep = false;
        rep = confirm(
            __("Are you sure you want to remove the selected records?")
        );
        if (rep) {
            delSelRecords();
        }
...
function downloadBasket() {
    ...
    open(
        loc,
        "win_form",
        "scrollbars=no,resizable=no,height=300,width=450,top=50,left=100"
    );
}
...
function updateBasket(updated_value, target) {
    if (target) {
        ...
        target
            .$("#cartDetails")
            .html(__("Records in your cart: %s").format(updated_value));
    } else {
        $("#basketcount").html(" <span>(" + updated_value + ")</span>");
        $("#cartDetails").html(
            __("Records in your cart: %s").format(updated_value)
        );
    }
}
