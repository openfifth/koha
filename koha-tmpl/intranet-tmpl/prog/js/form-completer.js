(function ($) {
    ...
    p._init = function _init() {
        ...
        var self = this;
        var $modal_form =
            this.opts.modal_form_id !== null
                ? $("#" + this.opts.modal_form_id)
                : $modal.parents("form");
            ...
            $input.each(function (index, element) {
                ...
                if (old != f.value) {
                    if ($(element).hasClass("flatpickr")) {
                        // check if it has the flatpickr class
    ...
    p.input_for = function inputs_for(f) {
        ...
        if (typeof f.attrname !== "undefined") {
            ...
            $form.each(function (index, form) {
                var names = [];
                $(form)
                    .find('[value="' + f.attrname + '"]')
                    .each(function (index, element) {
                        if (
                            element.name.startsWith(f.name) &&
                            element.name.endsWith("_code")
                        ) {
                            var name = element.name.substring(
                                0,
                                element.name.length - "_code".length
                            );
