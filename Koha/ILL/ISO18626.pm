...
sub confirmation_response {
    ...
    if ( $message_type eq 'request' ) {
        ...
        my $request = {
            ...
            requestingAgencyId        => $json->{request}->{header}->{requestingAgencyId}->{agencyIdValue},
        ...
        $response = {
            $confirmation_message_type => {
                confirmationHeader => {
                    timestamp         => DateTime->now( time_zone => 'UTC' )->strftime('%Y-%m-%dT%H:%M:%SZ'),
                    timestampReceived => DateTime->now( time_zone => 'UTC' )->strftime('%Y-%m-%dT%H:%M:%SZ'),
                    messageStatus     => 'OK',
                }
            }
        };
