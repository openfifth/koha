...
sub progress_request {
    ...
    if ($expectedDeliveryDate) {    #TODO: Ensure this datetime handling is done for every date field
        my $dt = eval { dt_from_string($expectedDeliveryDate) };
        if ($dt) {
            $expectedDeliveryDate = $dt->strftime('%Y-%m-%d %H:%M:%S');
        }
    }
