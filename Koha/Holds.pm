...
By default (no policy or count_as_group) delegates to count_holds(), counting
each hold group as one unit. When the policy is count_individually, counts
every hold row individually instead.

=cut

sub count_for_group_policy {
    my ( $self, $policy, $search_params, $search_attrs ) = @_;
    return ( $policy && $policy->rule_value eq 'count_individually' )
        ? ( $search_params ? $self->search( $search_params, $search_attrs )->count() : $self->count() )
        : $self->count_holds( $search_params, $search_attrs );
}
