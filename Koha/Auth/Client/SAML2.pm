...
sub _find_patron_by_matchpoint {
    ...
    my $count = $patron_rs->count;
    if ( $count > 1 ) {
        Koha::Logger->get->warn("SAML2: multiple patrons share $matchpoint='$value', matchpoints must be unique");
        return undef;
    }

    return $count ? $patron_rs->next : undef;
}
