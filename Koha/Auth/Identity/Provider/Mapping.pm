...
Override store to enforce the single-matchpoint rule and validate that at least
one of C<provider_field> or C<default_content> is supplied.
...
sub store {
    ...
    if ( $self->is_matchpoint ) {
        ...
        # Clear any existing matchpoint for this provider
        Koha::Auth::Identity::Provider::Mappings->search(
            {
                identity_provider_id => $self->identity_provider_id,
                is_matchpoint        => 1,
                ( $self->mapping_id ? ( mapping_id => { '!=' => $self->mapping_id } ) : () ),
            }
