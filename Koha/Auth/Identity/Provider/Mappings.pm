...
=head3 get_matchpoint
    ...
    my $matchpoint_mapping = $mappings->get_matchpoint;

Returns the mapping row flagged as the matchpoint, or undef if none.

=cut

sub get_matchpoint {
    my ($self) = @_;
    return $self->search( { is_matchpoint => 1 } )->next;
}
...
C<{ koha_field => provider_field, ... }> plus C<matchpoint> key.
...
sub as_auth_mapping {
    ...
    while ( my $m = $self->next ) {
        ...
        $matchpoint = $m->koha_field if $m->is_matchpoint;
    }

    return ( \%mapping, $matchpoint );
}
