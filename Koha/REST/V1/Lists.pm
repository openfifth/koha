...
sub delete {
    ...
    if (   $list->can_be_deleted( $user->id )
        || $list->allow_change_from_staff
        || $list->allow_change_from_others
        || ( $list->is_public && C4::Auth::haspermission( $user->userid, { lists => 'delete_public_lists' } ) ) )
    {
        return try {
            $list->delete;
