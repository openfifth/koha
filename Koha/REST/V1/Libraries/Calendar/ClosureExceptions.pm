package Koha::REST::V1::Libraries::Calendar::ClosureExceptions;
...
Controller function that handles adding a new closure exception.
...
sub add {
    ...
    return try {
        my $closure =
            Koha::Library::Calendar::Exception->new_from_api( { %{ $c->req->json }, library_id => $library->branchcode } );
        $closure->store;
        ...
        if ( blessed $_ and $_->isa('Koha::Exceptions::Object::DuplicateID') ) {
            return $c->render(
                status  => 409,
                openapi => { error => 'Closure already exists', error_code => 'duplicate' }
            );
        }
...
Controller function that handles retrieving a single closure exception.
...
Controller function that handles updating a closure exception.
...
sub update {
    ...
    return try {
        $closure->set_from_api( $c->req->json );
        $closure->store;
        return $c->render( status => 200, openapi => $c->objects->to_api($closure) );
    } catch {
        $c->unhandled_exception($_);
...
Controller function that handles deleting a closure exception.
