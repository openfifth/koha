package Koha::REST::V1::Libraries::Calendar::SingleClosures;
...
Controller function that handles adding a new single closure.
...
sub add {
    ...
    return try {
        my $closure =
            Koha::Library::Calendar::SingleClosure->new_from_api( { %{ $c->req->json }, library_id => $library->branchcode } );
        $closure->store;
...
Controller function that handles retrieving a single closure.
...
Controller function that handles updating a single closure.
...
Controller function that handles deleting a single closure.
