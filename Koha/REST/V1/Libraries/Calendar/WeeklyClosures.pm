package Koha::REST::V1::Libraries::Calendar::WeeklyClosures;
...
Controller function that handles adding a new weekly closure.
...
sub add {
    ...
    return try {
        my $closure =
            Koha::Library::Calendar::WeeklyClosure->new_from_api( { %{ $c->req->json }, library_id => $library->branchcode } );
        $closure->store;
...
Controller function that handles retrieving a single weekly closure.
...
Controller function that handles updating a weekly closure.
...
Controller function that handles deleting a weekly closure.
