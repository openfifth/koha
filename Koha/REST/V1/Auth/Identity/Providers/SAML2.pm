...
sub generate_certificate {
    ...
    unless ( $cn =~ /\S/ ) {
        return $c->render(
            status  => 400,
            openapi => { error => 'common_name is required' }
        );
    }

    # Sanitise: only allow characters valid in a CN / Subject (no shell metacharacters)
    if ( $cn =~ /[;<>&|`\$\\]/ ) {
        return $c->render(
            status  => 400,
            openapi => { error => 'common_name contains invalid characters' }
        );
    }
    ...
    unless ( grep { $_ == $key_size } ( 2048, 4096 ) ) {
        return $c->render(
            status  => 400,
            openapi => { error => 'key_size must be 2048 or 4096' }
        );
    }
    ...
    unless ( $days >= 1 && $days <= 3650 ) {
        return $c->render(
            status  => 400,
            openapi => { error => 'validity_days must be between 1 and 3650' }
        );
    }
    ...
    return try {
        ...
        unless ($ok) {
            ...
            return $c->render(
                status  => 500,
                openapi => { error => 'Certificate generation failed' }
            );
        }
