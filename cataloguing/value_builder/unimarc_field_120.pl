...
function Clic$function_name(event) {
    ...
    newin=window.open(\"/cgi-bin/koha/cataloguing/plugin_launcher.pl?plugin_name=unimarc_field_120.pl&index=\" + event.data.id + \"&result=\"+defaultvalue,\"unimarc_field_120\",'width=1200,height=750,toolbar=false,scrollbars=yes');
...
sub plugin {
    ...
    my ( $template, $loggedinuser, $cookie ) = get_template_and_user(
        {
            template_name => "cataloguing/value_builder/unimarc_field_120.tt",
            query         => $input,
            type          => "intranet",
