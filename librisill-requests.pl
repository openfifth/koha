...
my ( $template, $loggedinuser, $cookie, $flags ) = get_template_and_user(
    {
        template_name => "librisill-requests.tt",
        query         => $query,
        type          => "intranet",
...
# Build the url
my $url = "$protocol://$host/$string/$branch2/outgoing?start_date=$start" . "&end_date=$end";
...
my %lf_numbers = map { $_->{lf_number} } @{ $decoded->{ill_requests} };

my %users = map { $_->{user} } @{ $decoded->{ill_requests} };

my %titles = map { $_->{title} } @{ $decoded->{ill_requests} };

my %bib_ids = map { $_->{title} } @{ $decoded->{bib_id} };
